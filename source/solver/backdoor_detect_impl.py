...
################################################## Resnet - 34 https://github.com/bboylyg/NAD

def conv3x3RN34(in_planes, out_planes, stride=1):
    "3x3 convolution with padding"
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=False)

class BasicBlockRN34(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlockRN34, self).__init__()
        self.conv1 = conv3x3RN34(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3RN34(planes, planes)
...
class BottleneckRN34(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BottleneckRN34, self).__init__()
...
class ResNet34(nn.Module):
    ...
    def __init__(self, depth, num_classes=10, block_name='BasicBlock'):
        ...
        if block_name.lower() == 'basicblock':
            ...
            n = (depth - 2) // 6
            block = BasicBlockRN34
        elif block_name.lower() == 'bottleneck':
            assert (depth - 2) % 9 == 0, 'When use bottleneck, depth should be 9n+2, e.g. 20, 29, 47, 56, 110, 1199'
            n = (depth - 2) // 9
            block = BottleneckRN34
        else:
...
class SubResNet34(nn.Module):
    def __init__(
        self, block = BasicBlockRN34, num_classes=10,
    ):
        super().__init__()
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(64 * block.expansion, num_classes)
...
class BackdoorDetectImpl():
    ...
    def solve(self, model, assertion, display=None):
        ...
        elif dataset == 'cifar10_18' or dataset == 'cifar10_50':
        ...
        ######################## ResNet34 - NAD
            ...
            def __resnet(pretrained, depth, num_classes):
                model = ResNet34(depth, num_classes)
                if pretrained:
                    if file_name.endswith('tar'):
                        checkpoint = torch.load(file_name, map_location='cpu')
                        model.load_state_dict(checkpoint['state_dict'])
        ...
        #######################
            ...
            if dataset == 'cifar10_18':
                ...
                size_input, size_last = (3, 32, 32), 512 # 64 for RN34, 512 for RN18
            
            elif dataset == 'cifar10_34':
                file_name = './backdoor_models/cifar10_resnet34_bd.pth.tar'
                model = __resnet(pretrained=True, depth=32, num_classes=10)
                model.avgpool.register_forward_hook(get_activation(last_layer))
                sub_model = SubResNet34(block=BasicBlockRN34)
                size_input, size_last = (3, 32, 32), 64 # 64 for RN34, 512 for RN18
